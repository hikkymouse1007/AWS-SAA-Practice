...
## EBS Encryption
https://dev.classmethod.jp/articles/kms-encryption-ebs-bashscript/

- EBSを作成すると以下のデータが暗号化される。
    - ボリュームの中のデータ
    - インスタンスとボリュームでやり取りされるデータ
    - スナップショット
    - スナップショットから作成されるボリューム
- 暗号化と非暗号化は自動でされる
- 暗号化のレイテンシへの影響は低い
- 暗号化はKMSキーを利用している
- 非暗号化されたスナップショットのコピーは暗号化を許可する。

# EBSとInstance Storeの違い
https://blog.e2info.co.jp/2019/02/20/%E3%82%A4%E3%83%B3%E3%82%B9%E3%82%BF%E3%83%B3%E3%82%B9%E3%82%B9%E3%83%88%E3%82%A2%E3%83%9C%E3%83%AA%E3%83%A5%E3%83%BC%E3%83%A0%E3%82%A8%E3%83%95%E3%82%A7%E3%83%A1%E3%83%A9%E3%83%AB%E3%82%B9%E3%83%88/

- インスタンスの中には、RootEBSボリュームから復元できないものがある
- その代わりにインスタンスストアを使う
- インスタンスストア = 物理的にアタッチされる(EBSはネットワークドライブ)
- メリット
    - Better I/O
    - バッファ、キャッシュ、スクラッチデータ、一時的なコンテンツに良い
    - リブート時にはデータが残る
- デメリット
    - インスタンスの停止・終了でインスタンスストアは失われる
    - インスタンスストアのサイズ変更はできない
    - バックアップはユーザが手動で行う

例えば、c5dた.largeタイプのインスタンスにはストレージの追加画面にephemeral0というインスタンスストアが存在する。
